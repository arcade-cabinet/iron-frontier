...
import timeConfig from "@/config/game/time.json" with { type: "json" };
...
const AMBIENT: Record<DayNightPhase, { i: number; c: number }> = {
  dawn: { i: 0.35, c: 0xffaa66 },
  morning: { i: 0.55, c: 0xffeedd },
  midday: { i: 0.7, c: 0xfff5e6 },
  afternoon: { i: 0.55, c: 0xffe8cc },
  dusk: { i: 0.35, c: 0xff8844 },
  evening: { i: 0.2, c: 0x6655aa },
...
export class DayNightManager {
  ...
  setTimeMultiplier(v: number): void {
    this.realSecondsPerGameMinute = v;
  }
  getTimeOfDay(): number {
    return this.timeOfDay;
  }
  getPhase(): DayNightPhase {
    return this.currentPhase;
  }

  /** Sun angle in radians. 0 = horizon at 6 AM, PI/2 = zenith at noon. */
  getSunAngle(): number {
    return ((this.timeOfDay - 6) / 24) * Math.PI * 2;
  }
  getMoonAngle(): number {
  ...
  getNightFactor(): number {
    return computeNightFactor(this.timeOfDay);
  }

  getLighting(): LightingValues {
    const { current, next, t } = this.interpolate();
    const ca = AMBIENT[current],
      cb = AMBIENT[next];
    return {
