...
export class PipelineExecutor {
  ...
  constructor(apiKey: string, options?: { streamTimeoutMs?: number }) {
  ...
  /**
   * Resume a single pipeline step using an existing task ID
   */
  async resume(
    pipelineName: string,
    assetDir: string,
    ...
    manifest = await this.streamAndFinalize(step, pipeline, context, taskId);
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
    return manifest;
  }
  ...
  private async executeSingleStep(
    ...
    return this.streamAndFinalize(step, pipeline, context, taskId, iterationLabel);
  }
