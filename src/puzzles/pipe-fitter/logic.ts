...
export class PipeLogic {
  ...
  static getConnections(type: PipeType, rotation: Direction): [boolean, boolean, boolean, boolean] {
    ...
    return [rotated[0] === 1, rotated[1] === 1, rotated[2] === 1, rotated[3] === 1];
  }
  ...
  static checkFlow(state: PipePuzzleState): { solved: boolean; newGrid: PipeCell[][] } {
    ...
    while (queue.length > 0) {
      ...
      const currConns = PipeLogic.getConnections(currCell.type, currCell.rotation); // [N, E, S, W]
      ...
      for (let dir = 0; dir < 4; dir++) {
        ...
        if (currConns[dir]) {
          ...
          if (nx >= 0 && nx < width && ny >= 0 && ny < height) {
            ...
            const nextConns = PipeLogic.getConnections(nextCell.type, nextCell.rotation);
