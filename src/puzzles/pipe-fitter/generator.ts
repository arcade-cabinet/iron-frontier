import type { PipePuzzleState, PipeCell, PipeType, Direction } from './types';
...
export class PuzzleGenerator {
  ...
  static generate(width: number, height: number): PipePuzzleState {
    ...
    grid[startPos.y][startPos.x] = { ...grid[startPos.y][startPos.x], type: 'source', fixed: true, rotation: 1 }; // Pointing East
    grid[endPos.y][endPos.x] = { ...grid[endPos.y][endPos.x], type: 'sink', fixed: true, rotation: 3 }; // Pointing West
