...
export function getLocationTemplatesForBiome(biome: string): LocationTemplate[] {
  return LOCATION_TEMPLATES.filter(l =>
    l.validBiomes.length === 0 || l.validBiomes.includes(biome)
  );
}
...
export function getNpcCapacityForLocation(templateId: string): { min: number; max: number } {
  ...
  let minTotal = location.backgroundNpcRange[0];
  let maxTotal = location.backgroundNpcRange[1];

  // Add building NPC slots
  for (const buildingRef of location.buildings) {
    const building = getBuildingTemplateById(buildingRef.templateId);
    if (building) {
      const buildingMin = buildingRef.countRange[0];
      const buildingMax = buildingRef.countRange[1];
  ...
  // Add notable NPCs
  minTotal += location.notableNpcRange[0];
  maxTotal += location.notableNpcRange[1];
