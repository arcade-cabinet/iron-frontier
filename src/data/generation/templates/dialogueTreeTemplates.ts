...
function filterSnippetsByNPC(
  ...
  return snippets.filter(snippet => {
    ...
    if (snippet.validRoles.length > 0 && !snippet.validRoles.includes(npc.role)) {
      return false;
    }

    // Check faction restrictions
    if (snippet.validFactions.length > 0 && !snippet.validFactions.includes(npc.faction)) {
      return false;
    }

    // Check time of day restrictions
    if (snippet.validTimeOfDay.length > 0 && !snippet.validTimeOfDay.includes(timeOfDay)) {
      return false;
    }
