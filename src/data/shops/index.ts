...
export const ShopItemSchema = z.object({
  ...
  priceModifier: z.number().min(0).default(1.0),
  /** Minimum reputation to purchase */
  minReputation: z.number().int().default(-100),
...
export const ShopDefinitionSchema = z.object({
  ...
  buyModifier: z.number().min(0).max(1).default(0.5),
  /** Can player sell items here? */
  canSell: z.boolean().default(true),
  /** Item types accepted for sale */
  acceptedTypes: z.array(z.string()).default(['weapon', 'armor', 'consumable', 'junk']),
  /** Tags for filtering */
...
export function getShopsByTag(tag: string): ShopDefinition[] {
  return ALL_SHOPS.filter(shop => shop.tags.includes(tag));
}
...
export function calculateBuyPrice(shop: ShopDefinition, item: ShopItem): number {
  ...
  return Math.ceil(baseDef.value * item.priceModifier);
}
...
export function calculateSellPrice(shop: ShopDefinition, itemDef: BaseItem): number {
  return Math.floor(itemDef.value * shop.buyModifier);
}
...
export function getAvailableShopItems(
  ...
  return shop.inventory.filter(item => {
    ...
    if (item.minReputation > playerReputation) return false;
    // Check if hidden
    if (item.hidden) return false;
    // Check stock (0 = sold out)
...
export function canSellItemToShop(shop: ShopDefinition, itemType: string): boolean {
  ...
  return shop.acceptedTypes.includes(itemType);
}
