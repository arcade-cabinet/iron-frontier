...
function ActionButton({
  ...
  return (
    <button
      ...
      disabled={isDisabled}
      className={cn(
        'flex flex-col items-center gap-1 px-3 py-2 rounded-lg border border-stone-700/50 transition-all min-w-[70px]',
...
function EnemyCard({ combatant, isSelected, isTargetable, onSelect }: {
  ...
  return (
    <button
      ...
      className={cn(
        'relative p-3 rounded-lg border-2 transition-all min-w-[140px]',
    ...
    >
      ...
      {/* Name & Threat */}
      <div className="text-sm font-bold text-red-300 mb-2">{combatant.name}</div>
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        {healthPercent < 25 && (
          <span className="text-[10px] text-red-400 animate-pulse">Wounded</span>
        )}
      </div>
    </button>
  );
...
function PlayerCard({ combatant }: { combatant: Combatant }) {
  return (
    <div className="p-4 rounded-lg border-2 border-amber-600/50 bg-amber-950/90 backdrop-blur-sm min-w-[180px]">
      <div className="flex items-center gap-2 mb-2">
        <div className="w-8 h-8 rounded-full bg-amber-800/60 border border-amber-600/50 flex items-center justify-center">
          <svg className="w-4 h-4 text-amber-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
          </svg>
        </div>
        <div>
          <div className="text-amber-200 font-bold text-sm">{combatant.name}</div>
          <div className="text-amber-500/70 text-[10px]">Outlaw</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-2">
        <span className="text-[10px] text-amber-400/70">Action Points</span>
        <APBar current={combatant.actionPoints} max={combatant.maxActionPoints} />
      </div>
    </div>
  );
...
function CombatLog() {
  ...
  return (
    <div className="bg-stone-900/80 rounded-lg p-3 border border-stone-700/50 backdrop-blur-sm">
      <div className="text-[10px] font-medium text-amber-500/70 uppercase tracking-wide mb-2">Combat Log</div>
      <div className="space-y-1 min-h-[60px]">
        {recentLogs.length === 0 ? (
          <div className="text-stone-500 text-xs italic">The standoff begins...</div>
        ) : (
...
function OutcomeScreen({ type, onContinue }: {
  ...
  const config = {
    ...
    fled: {
      ...
      icon: <RunIcon className="w-16 h-16 text-stone-400" />,
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      <div className={cn(
        'rounded-lg border-2 p-8 text-center max-w-sm mx-4',
        borderColor,
        bgColor
      )}>
...
export function CombatPanel() {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex flex-col z-50">
      {/* Header */}
      <div className="bg-amber-950/90 border-b border-amber-800/50 px-4 py-3 backdrop-blur-sm">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-lg font-bold text-amber-400">Showdown</h2>
            <div className="text-xs text-amber-500/60">Round {combatState.round}</div>
          </div>
          <div className={cn(
      ...
      {/* Action Bar */}
      {isPlayerTurn && (
        <div className="bg-amber-950/90 border-t border-amber-800/50 p-3 backdrop-blur-sm">
          ...
          {combatState.selectedAction && !combatState.selectedTargetId && (
            <div className="text-center mt-2 text-amber-400 text-xs animate-pulse">
              Select a target...
            </div>
          )}
        </div>
      ...
      {!isPlayerTurn && combatState.phase === 'enemy_turn' && (
        <div className="bg-red-950/90 border-t border-red-800/50 p-4 backdrop-blur-sm">
