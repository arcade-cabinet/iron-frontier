...
export function WorldMap({ isOpen, onClose, onTravelTo }: WorldMapProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent
        side="bottom"
        className="h-[85vh] bg-amber-950 border-amber-700"
      >
        <SheetHeader className="pb-2">
          <SheetTitle className="text-amber-100 flex items-center justify-between">
            ...
            <span className="text-amber-400 text-sm font-normal">
              {discoveredLocationIds.length} / {world.locations.length} locations discovered
            </span>
          </SheetTitle>
        </SheetHeader>

        {/* Map Legend */}
        <div className="flex flex-wrap gap-4 mb-3 p-2 bg-amber-900/30 rounded-lg text-xs">
          <div className="flex items-center gap-1">
            <div className="w-4 h-0.5 bg-amber-800" />
            <span className="text-amber-300">Road</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-4 h-0.5 border-t-2 border-dashed border-amber-700" />
            <span className="text-amber-300">Trail</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-4 h-1 bg-gray-600" style={{ backgroundImage: 'repeating-linear-gradient(90deg, #4b5563 0px, #4b5563 2px, transparent 2px, transparent 6px)' }} />
            <span className="text-amber-300">Railroad</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full bg-red-600 border-2 border-amber-200" />
        ...
        <ScrollArea className="h-[calc(100%-120px)]">
          {/* SVG Map */}
          <div className="relative bg-[#f5f0e6] rounded-lg border-4 border-amber-800 shadow-lg overflow-hidden">
          ...
          <div className="mt-4 p-3 bg-amber-900/40 rounded-lg border border-amber-700/50">
                    ...
                    .map((conn) => {
                      ...
                      return (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          className="bg-amber-800/30 border-amber-700/50 text-amber-200 hover:bg-amber-700/50 justify-start text-left h-auto py-2"
