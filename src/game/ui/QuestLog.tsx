...
export function QuestLog() {
  ...
  return (
    <Sheet open={questLogOpen} onOpenChange={() => togglePanel('quests')}>
      <SheetContent side="bottom" className="h-[90vh] sm:h-[75vh] bg-amber-950 border-amber-700">
        <SheetHeader className="pb-1.5 sm:pb-2">
          <SheetTitle className="text-amber-100 text-base sm:text-lg">Journal</SheetTitle>
        </SheetHeader>

        <Tabs defaultValue="active" className="h-[calc(100%-52px)] sm:h-[calc(100%-60px)]">
          <TabsList className="w-full bg-amber-900/50">
            <TabsTrigger
              value="active"
              className="flex-1 min-h-[40px] sm:min-h-0 data-[state=active]:bg-amber-700 data-[state=active]:text-amber-100"
            >
