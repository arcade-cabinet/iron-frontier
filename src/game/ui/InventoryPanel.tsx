...
const TYPE_ICONS: Record<string, string> = {
  ...
  key_item: 'M21 10h-8.35C11.83 7.67 9.61 6 7 6c-3.31 0-6 2.69-6 6s2.69 6 6 6c2.61 0 4.83-1.67 5.65-4H13l2 2 2-2 2 2 2-2v-4zM7 15c-1.65 0-3-1.35-3-3s1.35-3 3-3 3 1.35 3 3-1.35 3-3 3z',
...
const getRarityBorder = (rarity: string) => {
  switch (rarity) {
    case 'legendary': return 'border-yellow-500/60 shadow-yellow-500/20 shadow-md';
    case 'rare': return 'border-purple-500/60';
    case 'uncommon': return 'border-green-500/60';
...
export function InventoryPanel() {
  ...
  return (
    <Sheet open={inventoryOpen} onOpenChange={() => togglePanel('inventory')}>
      <SheetContent side="bottom" className="h-[80vh] bg-amber-950 border-amber-800/60 p-0">
        {/* Header */}
        <SheetHeader className="p-3 pb-2 border-b border-amber-800/50">
        ...
        {/* Currency Bar */}
        <div className="flex items-center gap-4 px-3 py-2 bg-amber-900/20 border-b border-amber-800/30">
          <div className="flex items-center gap-1.5">
            <svg className="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 24 24">
              <circle cx="12" cy="12" r="10" />
            </svg>
            <span className="text-amber-200 font-bold text-sm">${playerStats.gold}</span>
          </div>
          {playerStats.ivrcScript > 0 && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-orange-300 font-bold text-sm">{playerStats.ivrcScript}</span>
            </div>
          )}
        </div>

        {/* Filter Tabs */}
        <div className="flex gap-1 px-3 py-2 bg-amber-900/10 border-b border-amber-800/30 overflow-x-auto">
        ...
        {/* Main Content */}
        <div className="flex gap-2 p-3 h-[calc(100%-140px)]">
          {/* Item Grid */}
          <ScrollArea className="flex-1">
            {filteredItems.length === 0 ? (
              <div className="text-amber-400/60 text-center py-12">
                <svg className="w-10 h-10 mx-auto mb-2 text-amber-700/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                </svg>
                <p className="text-sm">
                  {activeFilter === 'all' ? 'Your saddlebag is empty' : `No ${activeFilter === 'key_item' ? 'key items' : activeFilter + 's'}`}
                </p>
                {activeFilter === 'all' && (
                  <p className="text-xs mt-1 text-amber-500/40">Explore to find items!</p>
                )}
              </div>
            ) : (
                ...
                {filteredItems.map((item) => {
                  ...
                  return (
                    <Card
                      ...
                      className={cn(
                        'bg-amber-900/30 border cursor-pointer transition-all relative',
                    ...
                    >
                      <CardContent className="p-2">
                        ...
                        <div className="flex items-center gap-1.5 mb-1">
                          <svg
                            className={cn('w-3.5 h-3.5 flex-shrink-0', getTypeColor(item.type))}
                            fill="currentColor"
                            viewBox="0 0 24 24"
                          >
                            <path d={TYPE_ICONS[item.type] || TYPE_ICONS.junk} />
                          </svg>
                          <span className="text-amber-100 font-medium text-[11px] truncate flex-1">
                            {item.name}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          {item.quantity > 1 && (
                            <span className="text-amber-400/70 text-[10px] font-mono">
                              x{item.quantity}
                            </span>
                          )}
                          {item.type === 'weapon' && item.condition < 100 && (
                            <div className="flex-1 ml-1">
                              <div className="w-full h-1 bg-amber-900/60 rounded-full overflow-hidden">
                                <div
                                  className={cn(
                                    'h-full rounded-full',
          ...
          {/* Item Details Panel */}
          <div className="w-44 sm:w-52 bg-amber-900/20 rounded-lg p-2.5 flex flex-col border border-amber-800/30">
            {selectedItem ? (
              <>
                <div className="mb-2">
                  <div className="text-amber-100 font-bold text-sm mb-1 leading-tight">{selectedItem.name}</div>
                  <div className="flex items-center gap-1.5">
                    <Badge className={cn('text-[10px] px-1.5 py-0', getRarityColor(selectedItem.rarity))}>
                      {selectedItem.rarity}
                    </Badge>
                    {isItemEquipped(selectedItem.id) && (
                      <Badge className="text-[10px] px-1.5 py-0 bg-blue-600 text-blue-100">
                        Equipped
                      </Badge>
                    )}
                  </div>
                ...
                <div className="text-amber-400/70 text-[11px] mb-2 leading-relaxed flex-1 overflow-y-auto">
                  {selectedItem.description || 'No description available.'}
                </div>

                {/* Item stats */}
                <div className="space-y-0.5 text-[10px] text-amber-300/60 mb-2 pt-2 border-t border-amber-700/30">
                ...
                <div className="flex gap-1 flex-wrap">
                  {selectedItem.usable && (
                    <Button
                      ...
                      className="flex-1 h-7 text-[10px] bg-green-900/30 border-green-700/50 text-green-300 hover:bg-green-800/50"
                    >
                      Use
                    </Button>
                  )}
                  ...
                  {selectedItem.droppable && (
                    <Button
                      ...
                      className="flex-1 h-7 text-[10px] bg-red-900/20 border-red-700/30 text-red-400 hover:bg-red-800/30"
                    >
                      Drop
                    </Button>
                  )}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-amber-500/40 text-xs text-center">
                <svg className="w-8 h-8 mb-2 text-amber-700/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
