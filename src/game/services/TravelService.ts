 ...
 * and executing travel transitions. Works with the WorldLoader and gameStore.
 */

import { getTravelInfo, type LoadedWorld, type ResolvedLocation } from '../../data/worlds/index';
 ...
 * @param loadedWorld - Optional loaded world (defaults to store)
...
export function canTravel(
  ...
  const world = loadedWorld ?? useGameStore.getState().loadedWorld;
  ...
  if (!world.canTravelTo(fromId, toId)) {
    return false;
  }

  // Get target location
  const targetLocation = world.getLocation(toId);
  if (!targetLocation) {
    return false;
  }
...
export function getTravelCost(
  ...
  const travelInfo = getTravelInfo(world, fromId, toId);
  if (!travelInfo) {
    return null;
  }
...
export function getReachableLocations(
  ...
  const connections = world.getConnectionsFrom(fromId);
  const reachable: Array<{ location: ResolvedLocation; cost: TravelCost }> = [];

  for (const locRef of connections) {
    const location = world.getLocation(locRef.id);
    const cost = getTravelCost(fromId, locRef.id, world);
 ...
 */
export async function executeTravel(toId: string): Promise<TravelResult> {
  const store = useGameStore.getState();
  const { loadedWorld, currentLocationId, updateTime, travelTo } = store;
