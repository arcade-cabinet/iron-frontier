...
import { useGameStore, type InventoryItem } from '../store/webGameStore';
import { getItem } from '@iron-frontier/shared/data/items';
import { getItemTypeName } from '@iron-frontier/shared/data/schemas/item';
...
export function InventoryPanel() {
  ...
  return (
    <Sheet open={inventoryOpen} onOpenChange={() => togglePanel('inventory')}>
      <SheetContent side="bottom" className="h-[90vh] sm:h-[80vh] bg-amber-950 border-amber-800/60 p-0">
        {/* Header */}
        ...
        <div className="flex items-center gap-3 sm:gap-4 px-2 sm:px-3 py-1.5 sm:py-2 bg-amber-900/20 border-b border-amber-800/30">
          ...
          {playerStats.ivrcScript > 0 && (
            <div className="flex items-center gap-1 sm:gap-1.5">
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-400" fill="currentColor" viewBox="0 0 24 24">
                <rect x="4" y="4" width="16" height="16" rx="2" />
              </svg>
              <span className="text-orange-300 font-bold text-xs sm:text-sm">{playerStats.ivrcScript}</span>
            </div>
          )}
        </div>
          ...
          <ScrollArea className="flex-1 min-h-0">
            {filteredItems.length === 0 ? (
              <div className="text-amber-400/60 text-center py-8 sm:py-12">
                <svg className="w-8 h-8 sm:w-10 sm:h-10 mx-auto mb-2 text-amber-700/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {activeFilter === 'all' && (
                  <p className="text-[10px] sm:text-xs mt-1 text-amber-500/40">Explore to find items!</p>
                )}
              </div>
            ) : (
                    ...
                    >
                      <CardContent className="p-1.5 sm:p-2">
                        ...
                        <div className="flex items-center gap-1 sm:gap-1.5 mb-0.5 sm:mb-1">
                          <svg
                            className={cn('w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0', getTypeColor(item.type))}
                            fill="currentColor"
                            viewBox="0 0 24 24"
                          >
            ...
            {selectedItem ? (
              <>
                <div className="mb-1.5 sm:mb-2">
                  <div className="text-amber-100 font-bold text-xs sm:text-sm mb-1 leading-tight">{selectedItem.name}</div>
                  <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap">
                    <Badge className={cn('text-[9px] sm:text-[10px] px-1 sm:px-1.5 py-0', getRarityColor(selectedItem.rarity))}>
                      {selectedItem.rarity}
                    </Badge>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-amber-500/40 text-[10px] sm:text-xs text-center">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 mb-2 text-amber-700/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
