...
export function GameHUD() {
  ...
  return (
    <div className="absolute top-0 left-0 right-0 p-1 sm:p-2 pointer-events-none">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 pr-2 sm:pr-3 border-r border-amber-700/40">
              ...
              <div className="min-w-0">
                <div className="text-amber-100 font-bold text-[10px] sm:text-xs truncate max-w-[50px] sm:max-w-[80px]">
                  {playerName}
                </div>
                <div className="flex items-center gap-1 text-amber-400/80 text-[9px] sm:text-[10px]">
            ...
            <div className="flex-1 flex items-center gap-2 sm:gap-4">
              {/* Health - always visible but responsive width */}
              <div className="flex items-center gap-1 sm:gap-1.5 min-w-[70px] sm:min-w-[100px]">
                <HeartIcon className={getHealthColor()} />
                <div className="flex-1">
                  <Progress
                    value={healthPercent}
                    className={cn('h-1 sm:h-1.5 bg-amber-900/60', getHealthBarColor())}
                    aria-label="Health"
                  />
                </div>
                <span
            ...
            {/* Gold - Right */}
            <div className="flex items-center gap-1 sm:gap-1.5 pl-2 sm:pl-3 border-l border-amber-700/40">
              <CoinIcon className="text-yellow-500" />
              <span className="text-yellow-400 font-bold text-xs sm:text-sm min-w-[32px] sm:min-w-[40px] text-right">
                {playerStats.gold}
              </span>
            </div>
          </div>
      ...
      <div className="flex items-start justify-between mt-1 sm:mt-1.5 gap-2">
        {/* Current Location */}
        <div className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-amber-950/70 backdrop-blur-sm rounded border border-amber-800/40">
          <CompassIcon className="text-amber-500" />
          <span className="text-amber-200 text-[9px] sm:text-[10px] font-medium truncate max-w-[100px] sm:max-w-[150px]">
            {locationName}
          </span>
        </div>
